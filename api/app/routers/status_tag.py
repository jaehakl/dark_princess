...
from db import Status, StatusTag, Tag, get_db
...
async def api_get_status_tag_list(
    ...
    return await get_list_response(db, request, STATUS_TAG_CRUD_SPEC)
...
async def api_upsert_status_tag_list(
    ...
    await require_existing_ids(db, StatusTag, field_ids(items, "id"), "id")
...
async def api_delete_status_tag_list(
    ...
    await delete_owned_items(db, STATUS_TAG_CRUD_SPEC, ids)
    return None
