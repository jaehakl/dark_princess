...
from db import SceneDecision, SceneHistory, SceneOption, get_db
...
async def api_get_scene_decision_list(
    ...
    return await get_list_response(db, request, SCENE_DECISION_CRUD_SPEC)
...
async def api_upsert_scene_decision_list(
    ...
    await require_existing_ids(db, SceneDecision, field_ids(items, "id"), "id")
...
async def api_delete_scene_decision_list(
    ...
    await delete_owned_items(db, SCENE_DECISION_CRUD_SPEC, ids)
    return None
