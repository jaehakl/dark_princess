...
export const dbTables = {
  ...
  Scene: {
    ...
    columns: {
      ...
      repeat_policy: { label: '반복 정책', type: 'text', required: true, options: [{ key: 'always', label: '매번' }, { key: 'once_per_status', label: 'Status당 1회' }] },
  ...
  SceneTriggerBlock: {
    ...
    columns: {
      ...
      chance_percent: { label: '확률(%)', type: 'int' },
  ...
  SceneCondition: {
    ...
    columns: {
      ...
      kind: { label: '종류', type: 'text', required: true, options: [{ key: 'target', label: '대상' }, { key: 'status_tag', label: 'Status 태그' }, { key: 'target_tag', label: 'TargetStatus 태그' }, { key: 'scene_seen', label: '장면 확인' }, { key: 'status_stat', label: 'Status 수치' }, { key: 'target_interaction', label: 'Target 상호작용' }] },
